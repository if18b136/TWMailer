...
void *test_thread(void *arg) { //needs the socket connection parameters as argunments
	...
	// bool for sending welcome msg only once
	bool hello = false;
	...
	// Look if the connnected IP is in the block list
	for(pair<string, long int> elem : blockMap){
		...
		if(elem.first == data_p->ip_addr){
			if(timeCheckL < elem.second){
				...
				close(new_socket);
				pthread_exit(NULL);
				break; // should not  be necessary anymore
			}
			else{
	...
	// Check if the 3 wrong login attempts have been done
	// Running the check directly after the threading prevents clients from closing and opening the programm
	// in order to fake a clean west
	for(pair<string, string> elem : logMap){	// run through all logMap entries
		if(elem.first == data_p->ip_addr && elem.second == "false"){
			// count number of failed login attempts for a certain IP address
			ip_cnt++;
		}
	}
	if(ip_cnt >= 3){
		// delete previous inserts of ip address
	...
	do {
		...
		if( size > 0){
			...
			token = strtok (buffer,"\n");
			cout << endl << "Command received: "<< token << endl;
			...
			if(token == "LOGIN"){
				// Check if IP is already in blockMap
				for(pair<string, long int> elem : blockMap){
					time_t timeCheck = time(nullptr);
					long int timeCheckL = static_cast<long int>(timeCheck);
					if(elem.first == data_p->ip_addr){
				...
				// Check if IP has 3 faield login attempts
				...
				for(e = ldap_first_entry(ld, result); e != NULL; e = ldap_next_entry(ld,e)){
					...
					if(strncmp(input_uid.c_str(), cmp_uid.c_str(), 8)==0){
						...
						if (rc != LDAP_SUCCESS){
							...
							fprintf(stderr,"Credential login unsuccessful, %s\n",ldap_err2string(rc));
							strncpy (buffer,"ERR\n", BUF);
							send(new_socket, buffer, strlen(buffer),0);
							clear_buffer(buffer);
							//ldap_unbind_ext_s(ld, NULL, NULL);
